...
{
    echo "Installing WeAreOneBot unit file"
    MCNPM="$( command -v npm -v )"

    SERVICE=$(<$SCRIPTPATH/WeAreOneBot.service)
    ...
    echo "$SERVICE" | sudo tee /etc/systemd/system/WeAreOneBot.service > /dev/null
    sudo systemctl daemon-reload
    sudo systemctl enable WeAreOneBot
}
...
start_WeAreOneBot() {
    ...
    echo "Start WeAreOneBot, please make sure you configured the Bot correctly!"
    sudo systemctl start WeAreOneBot
}
